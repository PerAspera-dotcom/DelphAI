...
export default function Home() {
  ...
  async function send(text: string, isCustomInReader: boolean = false) {
    ...
    const hasPositionMarker =
      /(i think|i believe|i feel|i consider|in my view|ik denk|ik geloof|ik voel|je pense|je crois|ich denke|ich glaube|i would say|i argue)/i.test(
        trimmed,
      );
    ...
    return (
      <div className={styles.ageGate}>
        ...
        <div className={styles.ageBtns}>
          <button className={styles.ageYes} onClick={() => setAgeVerified(true)}>
            I agree
          </button>
          <button className={styles.ageNo} onClick={() => setAgeVerified(false)}>
            I don&apos;t agree
          </button>
        </div>
      </div>
        ...
        <div className={styles.modeCards}>
          ...
          >
            ...
            <div className={styles.modeCardDesc}>
              A Socratic dialogue that challenges your thinking. You form positions and defend them against
              philosophical frameworks from all traditions.
            </div>
          ...
          >
            ...
            <div className={styles.modeCardNote}>
              Type your own response at any point to switch to Philosopher mode.
            </div>
          </button>
        </div>
      </div>
              ...
              {sugs && activeMode === 'reader' && i === messages.length - 1 && (
                <div className={styles.readerSuggestions}>
                  ...
                  <button
                    ...
                    onClick={() => handleSuggestionClick(sugs.affirmative)}
                  >
                    {sugs.affirmative}
                  </button>
                  <button
                    className={`${styles.readerSugBtn} ${styles.readerSugNegate}`}
                    onClick={() => handleSuggestionClick(sugs.negative)}
                  >
                    {sugs.negative}
                  </button>
                  {sugs.more.map((m, j) => (
